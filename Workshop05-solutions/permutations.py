...
# A recursive function to calculate how many ways a number 
# of items can be arranged in a limited number of spaces.
#
def permutations(num_items, spaces):

    if spaces == 0: # Base case: No spaces so only one arrangement
        return 1

    else: # Recursive case: Some spaces to fill
        # Return the number of items that can be placed in the
        # first space multiplied by the number of ways all but 
        # the first item can be arranged in one less space
        return num_items * permutations(num_items - 1, spaces - 1)
