...
#
from string_module import *


#---------------------------------------------------------
# Given a string, return it in telegram format, i.e., all
# in caps, with full-stops replaced with "STOP" and
# spaces removed.
#
def telegram(message):
     return convert_to_upper(replace_stops(remove_spaces(message)))
    
## Alternatively, in separate steps...
##    message_spaces_removed = remove_spaces(message)
##    message_stops_replaced = replace_stops(message_spaces_removed)
##    message_upper = convert_to_upper(message_stops_replaced)
