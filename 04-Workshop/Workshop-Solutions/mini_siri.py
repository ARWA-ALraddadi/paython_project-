...
# A function which issues a different response depending on
# the contents of a given string
#
def respond(text):
    if '?' in text:
...
users_input = input("Hello, I'm Mini Siri. ")
# Repeatedly reply to the user a certain number of times
for reply in range(5):
    # Decide what Mini Siri should say
    siris_output = respond(users_input)
    # Get the next input from the user
    users_input = input(siris_output + " ")
